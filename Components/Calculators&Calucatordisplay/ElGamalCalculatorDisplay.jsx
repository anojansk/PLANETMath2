...
import ElGamalCalculator from "./ElGamalCalculator"
...
const inp = 
   ...
   const calculations = (message, field, binOperation,generator) && (
    (Number(message) < Number(field) && Number(message) > 0) ? 
                        <ElGamalCalculator
                            message = {Number(message)} 
                            field={Number(field)}
                            binOperation = {binOperation}
                            generator={Number(generator)}   
                        /> : <h2>Message must be lower than field and higher than 0!</h2> 
    )

    return(
        <>
    <h3>ElGamal Calculator</h3>
